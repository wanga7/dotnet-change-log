...
public static class GitTagExtensions
{
    ...
    public static Result<VersionInfo> ExtractVersionInfo(this Tag tag)
    {
        Match match = VersionTagPattern.Match(tag.FriendlyName);

        if (!match.Success)
        {
            return Result.Failure<VersionInfo>($"cannot match regex \"{VersionTagPattern}\"");
        }
        ...
        return Result.Success(new VersionInfo(prefix, version, suffix));
    }
